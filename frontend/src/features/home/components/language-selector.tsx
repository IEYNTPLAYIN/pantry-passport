...
export function LanguageSelector({ value, onChange }: LanguageSelectorProps) {
  ...
  const [activeIndex, setActiveIndex] = useState(() => supportedLanguages.indexOf(value));
  ...
  function handleKeyDown(event: React.KeyboardEvent<HTMLButtonElement>) {
    ...
    if (event.key === "ArrowDown" || event.key === "ArrowUp") {
      ...
      setActiveIndex((current) => {
        ...
        return (current + direction + supportedLanguages.length) % supportedLanguages.length;
      });
      return;
    }
  ...
  return (
    <div ref={containerRef} className="relative z-30 w-full sm:w-56">
      <button
        type="button"
        ...
        aria-activedescendant={isOpen ? `${listboxId}-${supportedLanguages[activeIndex]}` : undefined}
      ...
      >
        <span className="grid size-8 shrink-0 place-items-center rounded-full bg-meadow/10 text-meadow transition group-hover:bg-meadow/15">
          <svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className="size-[1.15rem]">
            <circle cx="12" cy="12" r="9" />
            <path d="M3 12h18M12 3a14.4 14.4 0 0 1 0 18M12 3a14.4 14.4 0 0 0 0 18" />
          </svg>
        </span>
        <span className="ml-3 min-w-0 flex-1">
          <span className="block text-[0.65rem] font-semibold uppercase tracking-[0.16em] text-ink/45">Language</span>
          <span className="block truncate text-sm font-semibold text-ink">{languageNames[value]}</span>
        </span>
        <svg
          aria-hidden="true"
          viewBox="0 0 20 20"
        ...
        >
          {supportedLanguages.map((language, index) => {
            ...
            return (
              ...
              >
                <span className={`mr-3 grid size-7 place-items-center rounded-lg text-[0.65rem] font-bold uppercase tracking-wider ${
                  isSelected ? "bg-white/15 text-white" : "bg-canvas text-meadow"
                }`}>
                  {language}
                </span>
                ...
                {isSelected ? (
                  <svg aria-hidden="true" viewBox="0 0 20 20" fill="currentColor" className="size-4">
                    <path fillRule="evenodd" d="M16.704 5.292a1 1 0 0 1 .004 1.414l-8 8.05a1 1 0 0 1-1.42 0l-4-4.025a1 1 0 0 1 1.42-1.41L8 12.63l7.296-7.334a1 1 0 0 1 1.408-.004Z" clipRule="evenodd" />
                  </svg>
                ) : null}
              </button>
            );
