...
export function PremiumAccessLink({
  ...
  if (isLoading) {
    return <div className="h-12 animate-pulse rounded-full bg-oat/35" aria-label="Loading subscription status" />;
  }
  ...
  return (
    ...
    >
      {isActive ? (
        ...
        <>
          <svg aria-hidden="true" viewBox="0 0 32 32" fill="none" className="mr-2 size-5 text-peach">
            <path d="m5 11 6 5 5-9 5 9 6-5-2.2 13H7.2L5 11Z" fill="currentColor" />
            <path d="M8 27h16" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" />
          </svg>
          {activateLabel}
