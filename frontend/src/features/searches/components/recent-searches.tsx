  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 px-3 py-2 text-xs font-semibold uppercase tracking-[0.16em] text-ink/45">
        <svg aria-hidden="true" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.6" className="size-4">
        ...
        <ul>
          {searches.map((search) => (
            <li key={search.id}>
              ...
              >
                <svg aria-hidden="true" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.6" className="size-4 shrink-0 text-ink/35 transition group-hover:text-meadow">
                  <path d="M10 5v5l3 2" strokeLinecap="round" strokeLinejoin="round" />
                  <circle cx="10" cy="10" r="7" />
                </svg>
                <span className="min-w-0 flex-1 truncate font-medium text-ink/80">{search.query}</span>
                <span className="rounded-md bg-canvas px-2 py-1 text-[0.65rem] font-semibold uppercase tracking-[0.14em] text-ink/45">{search.language}</span>
              </button>
            </li>
          ))}
        </ul>
